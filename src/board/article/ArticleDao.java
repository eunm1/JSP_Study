...
public class ArticleDao {
	...
	public ArrayList<Reply> getRepliesByArticleId(int id) {
		String sql = "SELECT * , m.nickname as writer FROM reply r INNER JOIN `member` m ON r.mid = m.id where aid = ?";
	...
	public ArrayList<Article> searchArticle(String dateInterval, String sTarget, String keyword) {
		...
		String sql = "SELECT a.*, m.nickname nickname, COUNT(l.aid) likeCnt FROM article a INNER JOIN `MEMBER` m ON a.mid = m.id LEFT JOIN `like` l ON a.id = l.aid WHERE ";
